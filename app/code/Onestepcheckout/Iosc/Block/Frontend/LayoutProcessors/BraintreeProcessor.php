...
class BraintreeProcessor implements \Magento\Checkout\Block\Checkout\LayoutProcessorInterface
{
    ...
    public function process($jsLayout)
    {
        ...
        $layoutPath = $jsLayout['components']['checkout']
                        ...
                        ['children']['payment']
                        ['children']['renders']
                        ['children']['braintree'] ?? false;

        if ($layoutPath) {

            $configKey = 'iosc_braintree';

            $layoutComponent = $layoutPath['component'] ?? false;

            if ($layoutComponent
                && strstr($layoutComponent, 'PayPal_Braintree') !== false
            ) {
                $component = 'Onestepcheckout_Iosc/js/view/payment/paypal_braintree';
            } else {
